...
import ChangeBillModal from '../modals/ChangeBillModal';
...
class Bills extends React.Component {
  ...
  onChangeBillModal(id, v, callback) {
    const currentItem = this.props.reward
      .filter(it => it.id == v.rewardType)[0]
      .lcMatchRewards.filter(it => it.itemid == v.rewardItem);
    const formData = {
      id,
      type: 1, // 话费
      worth: v.worth, // 商品价值
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        valueType: 'text',
        render: (_, row) =>
          row.type === 1 ? <Tag color="#108ee9">话费</Tag> : <Tag color="2db7f5">商品</Tag>,
      },
      {
        title: '商品价值',
        dataIndex: 'worth',
