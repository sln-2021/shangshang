...
import { StateType } from '../../../models/organizer';
...
// @ts-ignore
@Form.create()
class UnverifiedOrganizer extends React.Component {
  state = {
    error: '',
    isUpload: false,
  ...
  handleSubmit = e => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      ...
      easyDispatch(this, flag ? 'organizer/addOrganizer' : 'organizer/changeOrganizer', {
        // 普通字符串数据 key:values
        ..._.pick(
          values,
          'accountNumber',
        ...
        id: getId('conferenceId'),
        // 二进制文件数据
        logoFile:
          values.logoFile && values.logoFile.fileList[0]
            ? values.logoFile.fileList[0].originFileObj
            : null,
        businessLicenseFile:
          values.businessLicenseFile && values.businessLicenseFile.fileList[0]
            ? values.businessLicenseFile.fileList[0].originFileObj
            : null,
        corporateFile:
          values.corporateFile && values.corporateFile.fileList[0]
            ? values.corporateFile.fileList[0].originFileObj
            : null,
        corporateFanFile:
          values.corporateFanFile && values.corporateFanFile.fileList[0]
            ? values.corporateFanFile.fileList[0].originFileObj
            : null,
        // 回调方法处理事务
  ...
  isFirstLanding() {
    ...
    const flag = userType === 'UNAUTHCONFERENCEUSER' ? true : false;
  ...
  */
  ...
  render() {
    const IconFont = Icon.createFromIconfontCN({
      scriptUrl: '//at.alicdn.com/t/font_1625968_d6hvs7qs18.js',
    });
    const simpleRules = (message: string) => [{ required: true, message }];
    const {
      error,
      isUpload,
      isBusinessLicenseButton,
      isCorporateFileButton,
      isCorporateFanFileButton,
    } = this.state;
    const { loadingfetch, loadingAdd, loadingChange, form, organizerDetail } = this.props;
        ...
        <PageHeaderWrapper
          content={
            <div>
              当前审核状态：
              <Tag>
                {organizerDetail.status
                  ? `${ORGANIZER_STATE_MAP[organizerDetail.status].name} ${
                  ORGANIZER_STATE_MAP[organizerDetail.status].icon
                  }`
                  : '待提交资料'}
              </Tag>
              <div>请耐心等待管理员审核，当管理员拒绝时可以修改审核信息.</div>
            </div>
          }
          extraContent={
            isBtnDisplay ? (
              <Button loading={loadingAdd || loadingChange} type="primary" htmlType="submit">
                提交
              </Button>
            ) : null
          }
          ...
          <Card title="图标" bordered={false} loading={loadingfetch} style={{ marginBottom: 24 }}>
            <Row gutter={16} style={{ marginTop: 24, marginBottom: 24 }}>
              <Col span={12}>
                <AvatarIcon
                  size={54}
                  iconURL={organizerDetail.logoUrl || ''}
              ...
              <Col lg={6} md={12} sm={24}>
                <Form.Item label="渠道名称">
                  {form.getFieldDecorator('name', {
                    initialValue: organizerDetail.name || '',
              ...
              <Col lg={6} md={12} sm={24}>
                <Form.Item label="公司地址">
                  {form.getFieldDecorator('corporateAddress', {
                    initialValue: organizerDetail.corporateAddress || '',
              ...
              <Col lg={6} md={12} sm={24}>
                <Form.Item label="法人邮箱地址">
                  {form.getFieldDecorator('corporateEmail', {
                    initialValue: organizerDetail.corporateEmail || '',
              ...
              <Col lg={6} md={12} sm={24}>
                <Form.Item label="法人名称">
                  {form.getFieldDecorator('corporateName', {
                    initialValue: organizerDetail.corporateName || '',
              ...
              <Col lg={6} md={12} sm={24}>
                <Form.Item label="法人证件号">
                  {form.getFieldDecorator('corporateId', {
                    initialValue: organizerDetail.corporateId || '',
              ...
              <Col lg={6} md={12} sm={24}>
                <Form.Item label="银行名称">
                  {form.getFieldDecorator('bankName', {
                    initialValue: organizerDetail.bankName || '',
                    rules: simpleRules('请输入银行名称'),
                  })(<Input prefix={<IconFont type="icon-yinxing" />} style={{ width: '100%' }} />)}
                </Form.Item>
              </Col>
              <Col lg={6} md={12} sm={24}>
                <Form.Item label="银行账号">
                  {form.getFieldDecorator('accountNumber', {
                    initialValue: organizerDetail.accountNumber || '',
                    rules: simpleRules('请输入银行账号'),
                  })(
                    <Input prefix={<IconFont type="icon-yinlian1" />} style={{ width: '100%' }} />,
                  )}
                </Form.Item>
              </Col>
              <Col lg={12} md={12} sm={24}>
                <Form.Item label="开户支行">
                  {form.getFieldDecorator('bankBranch', {
                    initialValue: organizerDetail.bankBranch || '',
          ...
          >
            <Row gutter={16} style={{ marginTop: '24px' }}>
              <Col lg={6} md={12} sm={24}>
                <Row>
                  <Pictures
                    pictureURL={organizerDetail.businessLicenseUrl || ''}
              ...
              <Col lg={6} md={12} sm={24}>
                <Row>
                  <Pictures
                    pictureURL={organizerDetail.corporateUrl || ''}
              ...
              <Col lg={6} md={12} sm={24}>
                <Row>
                  <Pictures
                    pictureURL={organizerDetail.corporateFanUrl || ''}
...
export default connect(
  ({
    ...
    organizerDetail,
    loadingAdd: loading.effects['organizer/addOrganizer'],
    loadingChange: loading.effects['organizer/changeOrganizer'],
    loadingFetch: loading.effects['organizer/fetchOrganizerDetail'],
  }),
)(UnverifiedOrganizer);
