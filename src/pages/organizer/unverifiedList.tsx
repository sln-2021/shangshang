...
import ChangeOrganizerState from './modals/ChangeOrganizerState';
import NewOrganizerUser from './modals/NewOrganizerUser';
...
// @ts-ignore
@connect(({ organizer, user }) => ({ organizer, user }))
export default class Organizer extends React.Component {
  ...
  onChangeOrganizerState(id, v, callback) {
    this.clearError();
  ...
  onDeleteOrganizer(id) {
    this.clearError();
    easyDispatch(this, 'organizer/DeleteOrganizer', {
      id,
  ...
  onAddOrganizerUser(v, callback) {
    this.clearError();
    // @ts-ignore
    console.log(v);
    easyDispatch(this, 'user/addUsers', {
      ..._.pick(v, 'username', 'password', 'bz', 'storeId', 'conId', 'storeUrl', 'authFee'),
      userStatus: 0, // 主办方添加渠道时传入状态
      onError: code => {
        callback(false); // don't close
        this.onModalError(code);
      },
  ...
  onSkip(id, e) {
    e.preventDefault();
    const param = Base64.encodeURI(JSON.stringify({ back2: '/organizer-manager/unverifiedList' }));
    // @ts-ignore;
  ...
  render() {
    ...
    const columns: ProColumns<GithubIssueItem>[] = [
      ...
      },
      {
        ...
        render: (_, row) => (
          <Progress
            percent={ORGANIZER_STATE_MAP[Number(row.status)].progress}
            format={() =>
      ...
      },
      {
        ...
        render: (text, row, _, action) => (
          <Fragment>
            <span>
              <ChangeOrganizerState
                error={modalError}
                info={row}
                onShowAsync={() => this.clearError()}
                onOk={this.onChangeOrganizerState.bind(this, row.id)}
              >
            ...
            <span style={{ marginLeft: 10 }}>
              <Popconfirm
                ...
                onConfirm={this.onDeleteOrganizer.bind(this, row.id)}
                okType="danger"
                okText="确认"
                cancelText="取消"
              >
    ...
    return (
      <PageHeaderWrapper
        ...
        extraContent={
          <NewOrganizerUser
            onShowAsync={() => this.clearError()}
            onOk={this.onAddOrganizerUser.bind(this)}
            error={modalError}
          />
        }
      >
              ...
              <ProTable<GithubIssueItem>
                ...
                request={async (params = {}) => {
                  ...
                  const [beginTimeL, endTimeL] = rangePicker
                    ...
                    : [null, null];
                  // 1.执行 effects 方法获取数据
                  await easyDispatch(this, 'organizer/fetchUnverified', {
                    pageNum: current,
                    pageSize,
                    beginTimeL,
                    endTimeL,
                  ...
                  // 2.获取列表数据
                  const { unverifiedList } = await this.props.organizer;
                  return {
