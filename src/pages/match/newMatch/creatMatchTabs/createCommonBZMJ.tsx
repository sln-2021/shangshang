...
class CreateCommonBZMJ extends React.Component<P, S> {
  state = {
    error: '',
    formValues: {
      // name: Mock.mock('@cname'),
      // games: Mock.mock('@integer(1, 100)'),
      // players: Mock.mock('@string("1234", 1)'),
      ...
      // ['rewardType-4']: Mock.mock('@string("1", 1)'),
      // ['rewardItem-4']: Mock.mock('@string("123", 1)'),
      // ['rewardNumber-4']: Mock.mock('@integer(1000, 8000)'),
  ...
  handleSubmit = (e: React.FormEvent) => {
    ...
    this.props.form.validateFieldsAndScroll((err: unknown, values: any) => {
      ...
      const rewards: any = [
        ...
        {
          type: +values['rewardType-4'],
          itemid: +values['rewardItem-4'],
          num: +values['rewardNumber-4'],
        },
      ];
        ...
        <Card bordered={false} loading={false}>
          ...
          <Form.Item label="每桌人数">
            {getFieldDecorator('players', {
              initialValue: formValues.players,
          ...
          <Form.Item label="四个最高分奖励设定" required={false} style={{ marginBottom: 0 }}>
            <InputGroup key="1" style={{ width: '100%' }} compact>
              <Form.Item style={{ width: '30%', textAlign: 'center' }}>
                {getFieldDecorator(`rewardType-4`, {
                  validateTrigger: ['onChange', 'onBlur'],
                  initialValue: formValues['rewardType-4'],
                  rules: simpleRules('请选择奖励类型'),
                })(
                  <Select key="12" style={{ width: '100%' }} placeholder="请选择奖励类型">
                    {reward
              ...
              <Form.Item style={{ width: '35%', textAlign: 'center' }}>
                {getFieldDecorator(`rewardItem-4`, {
                  validateTrigger: ['onChange', 'onBlur'],
                  initialValue: formValues['rewardItem-4'],
                  rules: simpleRules('请选择奖励物品'),
                })(
                  <Select style={{ width: '100%' }} placeholder="请选择奖励物品">
                    {reward[getFieldValue('rewardType-4') - 1]
                      ? reward[getFieldValue('rewardType-4') - 1].lcMatchRewards.map(it => (
                          <Option value={it.itemid}>{it.itemName}</Option>
                        ))
                      : null}
                  </Select>,
              ...
              <Form.Item style={{ width: '35%', textAlign: 'center' }}>
                {getFieldDecorator(`rewardNumber-4`, {
                  validateTrigger: ['onChange', 'onBlur'],
                  initialValue: formValues['rewardNumber-4'],
                  rules: simpleRules('请输入奖励数量'),
                })(<InputNumber style={{ width: '100%' }} placeholder="数量" />)}
              </Form.Item>
            </InputGroup>
