...
import ChangeCommonBZModal from '../modals/ChangeCommonBZModal';
...
class CommonBZMJ extends PureComponent<P> {
  state = {
    error: '',
    modalError: '',
  };
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        valueType: 'text',
        hideInSearch: true,
        render: (_, row) => (row.costName ? `${row.costName} x${row.costN}` : '--'),
      },
      {
        title: '对局数',
        dataIndex: 'games',
      ...
      },
      {
        ...
        valueType: 'text',
        hideInSearch: true,
        render: (_, row) => (row.joinMin ? `${row.joinMin} ～ ${row.joinMax}` : '--'),
      },
      {
        title: '类型',
        dataIndex: 'matchType',
