...
class CreateMatchDDZ extends React.Component<P, S> {
  ...
  handleSubmit = (e: React.FormEvent) => {
    ...
    this.props.form.validateFieldsAndScroll((err: unknown, values: any) => {
      ...
      const id = location.search.split('=')[1];
      easyDispatch(this, 'games/updateMatch', {
        // 普通字符串数据
        ..._.pick(
          values,
          'competitionCharter',
  ...
  render() {
    ...
    const { loadingaddLandlordsMatch, reward, organizerTree, matchInfo } = this.props;
    const { isUpload, error, formValues } = this.state;
    const { status } = matchInfo;
    ...
    return (
      <PageHeaderWrapper title="斗地主比赛详情" content={<div>未审核的比赛可以进行修改</div>}>
...
export default connect(
  ({
    ...
    games,
    organizerTree,
    loadingaddLandlordsMatch: loading.effects['games/updateMatch'],
    reward: games.reward,
    matchInfo: games.matchInfo,
  }),
